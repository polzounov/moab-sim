...
class MoabModelOld:
    def __init__(self, linear_acceleration_servos, quantize):
    ...
    def step(self):
        ...
        """
        self.step_time = self.time_delta + MoabModelOld.random_noise(self.jitter)
    ...
    def accel_param(
        ...
        if self.linear_acceleration_servos:  # Use the servo linear acceleration
            return (q, vel)
        else:  # Skip all this stuff
            return (dest, vel)
    ...
    def update_plate(self, plate_reset: bool = False):
        ...
        # smooth transition to target based on accel and velocity limits
            ...
            theta_x, self.plate_theta_vel_x = self.accel_param(
                theta_x,
                theta_x_target,
            ...
            theta_y, self.plate_theta_vel_y = self.accel_param(
                theta_y,
                theta_y_target,
            ...
            z_pos, self.plate_vel_z = self.accel_param(
                z_pos,
                z_target,
                self.plate_vel_z,
                PLATE_Z_ACCEL,
            ...
            theta_x += MoabModelOld.random_noise(self.plate_noise)
            theta_y += MoabModelOld.random_noise(self.plate_noise)
    ...
    def _update_estimated_ball(self, ball: Vector3):
        ...
        # add the noise in
        self.estimated_x = x + MoabModelOld.random_noise(self.ball_noise)
        self.estimated_y = y + MoabModelOld.random_noise(self.ball_noise)
        ...
        self.estimated_direction = MoabModelOld.heading_to_point(
            self.estimated_x,
            self.estimated_y,
            self.estimated_vel_x,
            self.estimated_vel_y,
        ...
        self.obstacle_direction = MoabModelOld.heading_to_point(
            self.ball.x,
            self.ball.y,
            self.ball_vel.x,
            self.ball_vel.y,
...
class MoabEnvOld(Env):
    def __init__(self, linear_acceleration_servos=False, quantize=False):
        self.model = MoabModelOld(linear_acceleration_servos, quantize)
    ...
    def reset(self, config) -> None:
        ...
        info = self.get_state()
        state = [info["ball_x"], info["ball_y"], info["ball_vel_x"], info["ball_vel_y"]]
    ...
    def step(self, action):
        ...
        self.model.height_z = clamp(self.model.height_z, -1.0, 1.0)
        self.model.step()
        self.iteration_count += 1
        ...
        state = [info["ball_x"], info["ball_y"], info["ball_vel_x"], info["ball_vel_y"]]
        reward = 0
        return np.array(state), reward, self.halted(), info
